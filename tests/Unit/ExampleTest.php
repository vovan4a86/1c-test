...
class ExampleTest extends TestCase
{
    ...
    public function testInit()
    {
        ...
        $response->assertSeeText(
            function_exists("zip_open") ? 'zip=yes'
                : 'zip=no'
        );
    }
            ...
            while (! feof($file)) {
                ...
                if (! empty($session)) {
                    $sendSession = [
                        ...
                        'inputZipped'     => session('inputZipped'),
                    ];
                }
                ...
                $response->assertSessionDoesntHaveErrors('inputFolderName');
                // файлы, требующие распаковки
                $response->assertSessionDoesntHaveErrors('inputZipped');

                if (function_exists("zip_open")) {
                    $zip = session('inputZipped', []);
                    $this->assertArrayNotHasKey(File::basename($fileName), $zip);
                }
            }
            fclose($file);
        }
     ...
     * @return string
     */
    protected function createTmpFileToSend(): string
    {
...
//                $response->assertDontSeeText('failure');
    ...
    protected function setUp(): void
    {
        ...
        if (is_null($this->user)) {
            $this->user = factory(User::class, 'user')
                ->create(
                    [
                        'email' => config('protocolExchange1C.userEmailToTest'),
